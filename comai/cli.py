...
@click.option('--commit', '-c', is_flag=True, help='Commit the changes with the generated message')
@click.option('--template', '-t', default=os.getenv('TEMPLATE_COMMIT'), help='Specify a commit message template')
@click.option('--add', '-a', is_flag=True, help='Stage all changes before generating the commit message')
@click.option('--model', '-m', default='claude-opus', help='Set the engine model to be used')
def main(explanation, commit, template, add, model):
    ...
    system_message = "You are a helpful git commit assistant. You will receive a git diff and generate a commit message. Try to be meaningful and avoid generic messages."
    if template:
        system_message += f" The message should follow this template: {template}"

    user_message = formatted_diff
    if explanation:
        user_message = f"Here is a high-level explanation of the commit: {explanation}\n\n{user_message}"
